...
#    '/chat': {
#      target: 'http://localhost:8080',   ← githave-gateway 地址
#      changeOrigin: true,
#    }
...
> 完整接口文档见 `githave-gatway/README.md` 或启动后访问 Swagger：`http://localhost:8080/swagger/index.html`
...
2. **启动 githave-gateway**（端口 8080），在 `.env` 中配置 cfn_chatbot 的地址
3. **启动 mlops-front**（端口 3000），在 `webpack.dev.js` 中配置代理指向 gateway
4. 访问 `http://localhost:3000`，进入 AiChat 页面即可进行端到端对话
...
| 后端 API 文档 | 启动后访问 `http://localhost:8080/swagger/index.html` |
